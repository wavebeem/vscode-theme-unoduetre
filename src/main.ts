...
export class Theme {
  ...
  private themeTerminal() {
    const p = this.tintedAnsiDark(this.colorBG0, this.colorFG);
    return {
  ...
  private colors(): Record<string, string | undefined> {
    return {
      ...
      "widget.shadow": this.alpha(this.darken(this.colorBG1, 5), 50),
      ...this.themeScrollbar(),
  ...
  private tokenColors(): TokenColor[] {
    ...
    const dim = (foreground: string): string => {
      const bg = colord(this.colorBG0);
      let fg = colord(foreground);
      const delta = 0.1;
      if (bg.isDark()) {
        fg = fg.darken(delta);
        fg = fg.desaturate(delta);
      } else {
        fg = fg.lighten(delta);
        fg = fg.saturate(delta);
      }
      return this.fixContrast({
        fg: fg.toHex(),
        bg: this.colorBG0,
    ...
    const tokens = {
      ...
      dim: color(dim(this.colorFG)),
      uno: color(this.colorUno),
      unoDim: color(dim(this.colorUno)),
      unoBold: color(this.colorUno, "bold"),
      due: color(this.colorDue),
      dueDim: color(dim(this.colorDue)),
